...
async def login(user: Login):
    ...
    if security.check_password_hash(user_check.password, user.password):
        return HTTPException(status_code=status.HTTP_200_OK, detail=f"Successful {user.username}")

    return HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"wrong password or username")
...
async def register(user: Registration):
    ...
    if username is not None or email is not None:
            return HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="email error")
